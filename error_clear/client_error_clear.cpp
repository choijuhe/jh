...
void go_chatting(SOCKADDR_IN& client_addr) {     //채팅방 입장 함수
    ...
    while (1) {
        ...
        if (text == "/back") {
            ...
            int code1 = WSAStartup(MAKEWORD(2, 2), &wsa);
...
void MenuList(SOCKADDR_IN& client_addr, string* menu_id, string* menu_password) {
    ...
    case MENU_MODIFY:
        ...
        break;
    case MENU_CHAT:
        cout << "client_sock in menu: " << client_sock << endl;
...
void first_screen() {
    ...
    while (1) {
        ...
        if (client_choose == MAIN_LOGIN) { // 로그인
            ...
            cout << "client_sock in main: " << client_sock << endl;
...
int main() {
   // int client_choose = 0;
    ...
    pstmt = con->prepareStatement("TRUNCATE connecting_user;");
    pstmt->execute();
